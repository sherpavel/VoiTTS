// Package audio provisions a virtual microphone and streams PCM into it.
//
...
// Names of the devices this package creates. Descriptions avoid spaces so they
...
// DefaultMonitorLatency is the loopback buffer used for monitoring, in
// milliseconds. Low enough to feel immediate, high enough not to crackle.
const DefaultMonitorLatency = 50

// Options configures the virtual microphone.
type Options struct {
	// Keep leaves the devices loaded on Close, which suits a sink declared
	// persistently in the PipeWire config.
	Keep bool
	// Monitor mirrors the microphone's audio to the default output device, so
	// you hear what is being sent as it is sent.
	Monitor bool
	// MonitorLatency is the loopback buffer in milliseconds; zero selects
	// DefaultMonitorLatency. Bluetooth outputs need more headroom than wired
	// ones and may need this raised.
	MonitorLatency int
}

// Format describes a PCM stream. It mirrors tts.Format so the two packages
...
// VirtualMic is a null sink paired with the capture device that reads from it.
type VirtualMic struct {
	// Sink is the name to play audio into.
	Sink string
	// Source is the capture device other applications should select.
	Source string
...
func Open(ctx context.Context, opts Options) (*VirtualMic, error) {
	...
	m := &VirtualMic{Sink: SinkName, Source: SourceName, keep: opts.Keep}
...
func (m *VirtualMic) provision(ctx context.Context, opts Options) error {
...
func (m *VirtualMic) enableMonitor(ctx context.Context, latencyMS int) error {
	if latencyMS <= 0 {
...
// Stream is a live PCM connection to the virtual microphone, held open for the
// life of the program.
//
...
func (m *VirtualMic) OpenStream(ctx context.Context, f Format) (*Stream, error) {
	...
	return &Stream{cmd: cmd, in: in, name: m.playerKind, stderr: &stderr}, nil
}
...
func (m *VirtualMic) playerArgv(f Format) ([]string, error) {
...
func (m *VirtualMic) Close() error {
	if m == nil || m.keep || len(m.modules) == 0 {
		return nil
	}
